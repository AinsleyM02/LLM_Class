...
emb_size = 128  # * embedding size for each token #THIS HAS BEEN EXPANDED< GIVING MODEL FREEDOM TO LEARN MORE THAN DIM OF SPACE! 
batch_size = 64  # batch size for training and validation (determines how many samples are used in each iteration) # HOW MANY INPUTS GIVE MODEL AT ONCE THIS WAS INCREASED
block_size = 128  # * block size for training and validation (determines the sequence length of the input) # TO PREDICT THE 129th token, see what the last 128 were
...
model_args = dict(n_layer=num_layers, n_head=num_heads, n_embd=emb_size, block_size=block_size,
                  bias=False, vocab_size=None) # start with model_args from command line
...
# notice in this version the training loss is higher than the
# testing loss and so the model is overfitting. This is where we add 
# dropout to the model to prevent overfitting- the model starts to use the same  neurons to learn and 
# so we drop some out (cover them) and so the model must learn to use other neurons to learn the same thing
# we will make the model learn on diff neuronsA