...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, head_size):
        super().__init__()
        self.heads = nn.ModuleList([Head(head_size) for _ in range(num_heads)]) # using list of atttention heads, get output and get concatted output
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size):
        ...
        self.att_head = MultiHeadAttention(8, emb_size//8) # have 8 attention heads, so divide by 8 so can get original dim
    ...
    def forward(self, idx, targets=None):
        ...
        logits = self.linear_head(x)  # (B,T,vocab_size) # sent to the multi attention head
