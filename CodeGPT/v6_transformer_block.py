...
class TransformerBlock(nn.Module): # one trnsformer block
    def __init__(self, embd_size, num_heads):
        super().__init__()
        head_size = embd_size // num_heads
        self.att_head = MultiHeadAttention(num_heads, head_size) # thru multi ead attention
        self.feedforward = FeedForward(emb_size) # pass thru feedforward, will repeat this in the implementation
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size):
        ...
        self.blocks = nn.Sequential( # 4 blocks (repeated 4 times, output of prev block input to the next block)
            TransformerBlock(emb_size, num_heads=8),
            TransformerBlock(emb_size, num_heads=8),
            TransformerBlock(emb_size, num_heads=8),
            TransformerBlock(emb_size, num_heads=8),
        )
        self.linear_head = nn.Linear(emb_size, vocab_size) # output of final block is passed to here
